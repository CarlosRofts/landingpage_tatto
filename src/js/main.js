...
function init() {
  ...
  imgLoad.on('done', function (instance) {
    ...
    gsap.set(progressBar, {
      autoAlpha: 0,
      onComplete: () => {
        initPageTransitions({ loaderInner, loader, loaderMask, initContent: () => initContent(), initLoader });
      },
    });
  });
}
...
function animateNavigation() {
  ...
  function navAnimation(direction) {
    const scrollingDown = direction === 1; // is true if is one
...
function createPortfolioMove(e) {
  ...
  gsap.to(largeImage, { duration: 1.2, y: getPortfolioOffset(clientY) / 6, ease: 'Power3.out' });
  gsap.to(smallImage, { duration: 1.5, y: getPortfolioOffset(clientY) / 3, ease: 'Power3.out' });
}
